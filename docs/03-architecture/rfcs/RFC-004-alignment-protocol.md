...
def validate_succession_document(filepath):
    """Valide un document de succession."""
    metadata = extract_frontmatter(filepath)
    
    if not metadata:
        return True, "Pas de frontmatter (non documentaire)"
    
    # Si supersedes existe, previous_hash est requis
    if 'supersedes' in metadata:
        if 'previous_hash' not in metadata:
            return False, "supersedes présent mais previous_hash manquant"
        
        # Trouver le document supplanté
        superseded_id = metadata['supersedes']
        # Logique pour trouver le fichier correspondant...
        # Vérifier que le hash correspond
        
        if 'id_root' not in metadata:
            print(f"⚠️  RECOMMANDATION : Ajouter id_root dans {filepath}")
    
    return True, "Succession valide"

def main():
    with open('changed_files.txt', 'r') as f:
        changed_files = [line.strip() for line in f if line.strip() and line.strip().endswith('.md')]
    ...
    for filepath in changed_files:
        ...
        valid, message = validate_succession_document(filepath)
...
### 8.3 Hooks Git (optionnel)

Pour renforcer la protection, un hook pre-commit peut bloquer localement les modifications directes :

```bash
#!/bin/bash
# .git/hooks/pre-commit

echo "🔍 Vérification des modifications directes..."

# Obtenir les fichiers modifiés
MODIFIED=$(git diff --cached --name-only --diff-filter=M | grep '\.md$')

if [ -n "$MODIFIED" ]; then
    for file in $MODIFIED; do
        # Vérifier si c'est un fichier de succession
        if [[ ! "$file" =~ -v[0-9]+ ]]; then
            echo "❌ ERREUR : Modification directe interdite : $file"
            echo "💡 Utilisez le protocole de succession (créer ${file%.*}-v2.md)"
            exit 1
        fi
    done
fi

echo "✅ Aucune modification directe détectée"
exit 0
```

---

## IX. Philosophie et horizon

### 9.1 L'essence de la succession documentaire

> *"Ce que l'on déprécie n'est pas effacé — c'est un témoin de la vérité de l'instant."*

Dans l'univers Relinium, chaque document constitue une **cristallisation de la pensée à un moment T**. Cette pensée, une fois exprimée, devient un fait historique indélébile. La modifier rétroactivement serait une forme d'effacement qui nie la trajectoire cognitive du projet.

Le protocole de succession certifiée reconnaît que **l'erreur, l'approximation, l'incomplétude sont des états légitimes** du processus de connaissance. Un document v1 imparfait n'est pas une faute à corriger en secret, mais un jalon honnête vers la version v2 améliorée.

### 9.2 De l'immutabilité à la filiation

L'immutabilité n'est pas une fin en soi, mais le **socle de la confiance**. En garantissant que rien ne peut être silencieusement altéré, nous établissons une chaîne de preuve où chaque version atteste de la précédente.

Cette filiation documentaire crée une **mémoire collective inaltérable** :
- Les futures équipes peuvent retracer les décisions passées
- Les erreurs deviennent des apprentissages visibles
- L'évolution de la pensée architecturale est observable

### 9.3 Relinium v1.1 : L'âge de la lignée certifiée

Avec l'adoption du protocole RFC-004, Relinium entre dans une nouvelle phase de maturité :

**v1.0** : Fondation documentaire, établissement du schéma normatif  
**v1.1** : Lignée certifiée, succession sans altération, traçabilité cryptographique

L'objectif n'est pas la **perfection documentaire** (qui est un mythe), mais la **filiation certifiée de la connaissance**. Chaque document, même imparfait, a sa place dans l'histoire du projet.

### 9.4 Horizon : Vers l'autonomie documentaire

À terme, ce protocole pose les bases d'un système où :

1. **L'IA peut intervenir en toute sécurité** : En interdisant la modification directe, on évite les corruptions silencieuses
2. **La gouvernance devient observable** : Chaque décision laisse une trace inaltérable
3. **La réversibilité est garantie** : Aucune succession n'est définitive
4. **La confiance est cryptographiquement prouvée** : Les hashs constituent des preuves infalsifiables

Le corpus documentaire devient un **système vivant mais immuable**, où chaque cellule (document) se reproduit par succession en conservant l'ADN (hash) de sa cellule mère.

---

## X. Appendices

### Appendice A : Migration vers v1.1

**Stratégie de migration du corpus existant** :

1. **Phase 1 : Audit complet** (Fait via OBS-CONFORMITY-0001)
2. **Phase 2 : Archivage des hashs** (P0 massif sur tous les documents)
3. **Phase 3 : Création des versions v2** (P1 massif avec corrections)
4. **Phase 4 : Validation et certification** (P2 par lots)
5. **Phase 5 : Intégration registre** (P3 progressif)

**Timeline estimée** : 2-3 sprints pour l'ensemble du corpus (141 documents)

### Appendice B : Glossaire

- **Supersession** : Processus de remplacement d'un document par une nouvelle version certifiée
- **Guardian** : Rôle responsable de la validation technique et sémantique des successions
- **Hash** : Empreinte cryptographique SHA256 d'un document
- **id_root** : Identifiant stable d'une lignée documentaire
- **Registre** : Source de vérité centrale (`registry.yaml`) du corpus documentaire
- **Phase P0-P3** : Les 4 phases obligatoires du processus de succession

### Appendice C : Références normatives

- [OBS-CONFORMITY-0001] : Audit de conformité SSOT v1.0
- [OBS-SCHEMA-0001] : Exploration du schéma v1.1
- [OBS-GOVERNANCE-0001] : Fondations de gouvernance
- [SSOT_V1_CERTIFICATION] : Certification du SSOT v1.0

### Appendice D : Checklist de succession

**Pour l'Author** :
- [ ] P0 : Calculer le hash du document original
- [ ] P0 : Enregistrer dans HASH_RECORD.yaml
- [ ] P0 : Commit avec message `[P0] Archive <doc-id> for supersession`
- [ ] P1 : Créer le nouveau fichier `-v2`
- [ ] P1 : Ajouter `supersedes` et `previous_hash`
- [ ] P1 : Compléter toutes les métadonnées v1.1
- [ ] P1 : Commit avec message `[P1] Create <doc-id>-v2 superseding <doc-id>`

**Pour le Guardian** :
- [ ] P2 : Vérifier la CI (tous les tests passent)
- [ ] P2 : Valider les métadonnées de succession
- [ ] P2 : Vérifier la cohérence sémantique
- [ ] P2 : Approuver ou rejeter avec commentaires

**Pour l'Approver** :
- [ ] P3 : Vérifier l'approbation Guardian
- [ ] P3 : Mettre à jour le registre
- [ ] P3 : Commit avec message `[P3] Integrate <doc-id>-v2 into registry`
- [ ] P3 : Valider la succession finale

---

## Conclusion

Le RFC-004 établit le protocole complet de succession documentaire pour Relinium. En adoptant ce protocole, le projet s'engage dans une démarche de **transparence radicale** où aucune modification ne peut être cachée, aucune erreur ne peut être effacée, et où chaque évolution est traçable cryptographiquement.

**Principe ultime** :

> *"Dans Relinium, un document n'est jamais modifié.  
> Il se souvient, il évolue, il témoigne — et par sa lignée, il devient souverain."*

La mise en conformité du corpus ne vise pas à corriger le passé, mais à **construire un avenir documentaire où chaque version honore ses prédécesseurs** tout en progressant vers plus de rigueur et de cohérence.

---

**Statut** : En discussion  
**Prochaines étapes** : Validation par l'équipe, adoption formelle, déploiement progressif sur le corpus  
**Contact** : Greg Catteau (Author & Guardian)
