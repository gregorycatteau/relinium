...
const organizationType = ref('entreprise')
...
async function submitRequest(): Promise<void> {
  ...
  try {
    ...
    `, {
      input: {
        email: email.value,
        organizationHint: organizationName.value,
        requestedRole: 'admin',
        messageRedacted
      }
    })
...
<template>
  <section class="w-full bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
    <div class="mx-auto max-w-5xl">
      <div class="max-w-3xl">
        <h1 class="text-3xl font-semibold text-slate-950">Créer un compte entreprise</h1>
          ...
          <section class="min-w-0 rounded-lg border border-slate-200 bg-white p-5">
            ...
            <div class="mt-4 grid gap-4 sm:grid-cols-2">
              <label class="block text-sm font-semibold text-slate-700">Nom de l’organisation
                <input v-model="organizationName" class="mt-1 h-10 w-full rounded-md border border-slate-300 px-3 text-sm" type="text" required>
              </label>
        ...
        <aside class="space-y-5">
          ...
          <section class="min-w-0 rounded-lg border border-slate-200 bg-white p-5">
            <p class="text-sm leading-6 text-slate-700">
              Ne saisissez aucun mot de passe, token, clé API ou information bancaire. La demande prépare un échange de validation, elle n’ouvre pas automatiquement l’accès.
            </p>
